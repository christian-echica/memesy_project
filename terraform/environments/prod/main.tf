...
module "iam" {
  ...
  stripe_webhook_secret_ssm_arn = "arn:aws:ssm:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:parameter/memesy/prod/stripe-webhook-secret"
